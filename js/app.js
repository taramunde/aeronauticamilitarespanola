...
document.addEventListener('DOMContentLoaded', () => {
    ...
    tlBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if(dot) dot.classList.add('!bg-[#ffc400]');
            const data = tlData[btn.dataset.y];
            if(tlYear) tlYear.textContent = data.y;
            if(tlTitle) tlTitle.textContent = data.t;
            if(tlDesc) tlDesc.textContent = data.d;
            const panel = document.getElementById('timeline-panel');
            if(panel) panel.animate([{opacity:0.5, transform:'translateY(5px)'},{opacity:1, transform:'translateY(0)'}], {duration:300});
        });
    });
    if(tlBtns.length > 0) tlBtns[0].click();
    ...
    // Click en tarjetas de flota para abrir modal
    document.querySelectorAll('.flota-card').forEach(card => {
